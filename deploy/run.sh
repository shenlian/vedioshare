...
if [ $1 = 'start' ];then
    ...
    sudo cp vedioshare_server /etc/nginx/sites-available/vedioshare_server
    sudo ln -s /etc/nginx/sites-available/vedioshare_server /etc/nginx/sites-enabled/vedioshare_server
    sudo cp vedioshare.ini /etc/uwsgi/apps-available/
